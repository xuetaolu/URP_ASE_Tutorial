...
Shader "RampMap"
{
	Properties
	{
		...
		[ASEEnd][ASEBegin]_RampMap("RampMap", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float dotResult14 = dot( ase_worldNormal , _MainLightPosition.xyz );
				float nDotL60 = dotResult14;
				float2 appendResult16 = (float2(max( nDotL60 , 0.0 ) , 0.0));
				float ase_lightAtten = 0;
				Light ase_mainLight = GetMainLight( ShadowCoords );
				ase_lightAtten = ase_mainLight.distanceAttenuation * ase_mainLight.shadowAttenuation;
				float smoothstepResult62 = smoothstep( 0.0 , 0.01 , nDotL60);
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( tex2D( _RampMap, appendResult16 ) * ( ase_lightAtten * smoothstepResult62 ) ).rgb;
				float Alpha = 1;
...
ASEEND*/
//CHKSM=7EB762D65EE8E772AA3CDD8D3C5FD3024318FADA